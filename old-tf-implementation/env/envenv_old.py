...
class EnvEnv(gym.Env):
    ...
    def __init__(self, df, render_mode=None, size=5):
        ...
        self.observation_space = Box(-np.inf, np.inf, shape=(14,))
    ...
    def step_discrete(self, action):
        ...
        if action <10:
            ...
            if is_buy_possible:
                ...
                num_buying = min(num_buy_possible, abs(action))
                self.num_shares += num_buying
                self.balance -= num_buying*current_price
        
        #sell
        elif action >0 and self.num_shares>0 :
            # self.num_shares_sold +=1

            # self.num_shares -= 1
            # self.balance += current_price

            num_selling = min(self.num_shares, action)
    ...
    def reset(self):
        ...
        with open('./logs/log102training.txt', 'w') as log:
            log.writelines(self.training_output)
            log.close()
        
        with open('./logs/log102training-rewards.txt', 'w') as log:
            log.writelines(self.training_rewards)
            log.close()

        with open('./logs/log102training-actions.txt', 'w') as log:
